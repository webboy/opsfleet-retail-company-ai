...
3. Implement `bq.py`: `sql_guard` first (pure function → easy tests), then `BigQueryRunner` with `maximum_bytes_billed`, typed `QueryResult` (ok/error/empty + DataFrame).
4. Tests: pytest for `sql_guard` matrix (select ok, DML blocked, DDL blocked, multi-statement blocked, wrong table blocked, LIMIT injected).
