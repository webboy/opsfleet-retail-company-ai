...
- Extensibility section must cover **MCP as the production tool-integration mechanism**: in the prototype capabilities are LangGraph nodes/tools in-process; in production, tools like BigQuery access, the Golden Bucket and the reports library can be exposed as MCP servers so other agents/clients reuse them and new data sources plug in without changing the agent — include the trade-off reasoning (why not MCP in the prototype).
